...
signInPrize[1].Prize[1] = 65000;	--面包
signInPrize[1].Prize[2] = 65001;	--生命力回复药（100）
...
signInPrize[2].Prize[1] = 65000;	--面包
signInPrize[2].Prize[2] = 65001;	--生命力回复药（100）

signInPrize[2].Count = {}; --定义奖励数量表
signInPrize[2].Count[1] = 5;
signInPrize[2].Count[2] = 5;
...
signInPrize[3].Prize[1] = 9611;	--铜条
signInPrize[3].Prize[2] = 12402;	--盐


signInPrize[3].Count = {}; --定义奖励数量表
signInPrize[3].Count[1] = 20;
signInPrize[3].Count[2] = 20;
...
signInPrize[4].Prize[1] = 16001;	--白钥匙
signInPrize[4].Prize[2] = 16002;	--黑钥匙
signInPrize[4].Prize[3] = 45436;	--麦克风
...
signInPrize[5].Prize[1] = 60016;	--治愈之花


signInPrize[5].Count = {}; --定义奖励数量表
signInPrize[5].Count[1] = 1;
...
signInPrize[6].Prize[1] = 65002;	--声望果
signInPrize[6].Prize[2] = 65003;	--打卡器
signInPrize[6].Prize[3] = 65004;	--诱魔香Lv1
...
signInPrize[7].Prize[1] = 65005;	--时间水晶Lv1
signInPrize[7].Prize[2] = 65006;	--时间水晶Lv2
signInPrize[7].Prize[3] = 65004;	--诱魔香Lv1
signInPrize[7].Prize[4] = 65007;	--诱魔香Lv2

signInPrize[7].Count = {}; --定义奖励数量表
signInPrize[7].Count[1] = 1;
signInPrize[7].Count[2] = 1;
...
function signin_create() --每日签到
	if(SigninNPC == nil)then
		...
		Char.SetData(SigninNPC,%对象_形象%,14576);
		Char.SetData(SigninNPC,%对象_原形%,14576);
		Char.SetData(SigninNPC,%对象_X%,247);
		Char.SetData(SigninNPC,%对象_Y%,83);
		Char.SetData(SigninNPC,%对象_地图%,1000);
		Char.SetData(SigninNPC,%对象_方向%,6);
		Char.SetData(SigninNPC,%对象_名字%,"签到活动");
...
function SigninNPCTalked(_me,_tome)
	...
	if(signble == 1 and times == 0)then
		myStr = ""..(Char.GetData(_tome,%对象_名字%)).."你好，欢迎您来到炫斗魔力！\\n\\n".."....我是负责给大家记名签到的\\n\\n您似乎有一段时间都没有来过炫斗魔力了呢！不知道您近来过得可好呢？不管如何，希望您在炫斗魔力能愉快的玩耍！\\n\\n请问您想现在就签到（领取礼物）吗？";
		NLG.ShowWindowTalked(_tome,_me,%窗口_信息框%,%按钮_是否%,SIGNIN_MAIN,myStr);
		return;
	end
end
...
function makeSignin(_me,_tome,times)
	...
	if(prizeCount ~= -1)then
		...
		if(Char.ItemSlot(_tome) > 16)then
			NLG.ShowWindowTalked(_tome,_me,%窗口_信息框%,%按钮_关闭%,SIGNIN_END,"\\n\\n你的背包似乎装不下奖励了哦!\\n\\n快去清理下吧!");
			return;
		end
		local PrizeFlg = 0;
