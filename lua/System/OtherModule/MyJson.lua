...
local function objToStr(value, isArray)
    if type(value)=='table' then
        return MyJson.tblToStr(value, isArray)
...
local function tblToJson(key, value)
    if isArrayTable(value) then
        return array2json(key, value)
    end
    local tableStr = MyJson.tblToJson(value)
