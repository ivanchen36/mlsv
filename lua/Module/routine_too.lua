...
routine_list[1] = {};
routine_list[1].type = 2;
routine_list[1].target = "火蜘蛛,333,80";
routine_list[1].reward = "0|0|0|每日任务礼包,588451,1";

routine_list[2] = {};
routine_list[2].type = 2;
routine_list[2].target = "丧尸,102,80";
routine_list[2].reward = "0|0|0|每日任务礼包,588451,1";

routine_list[3] = {};
routine_list[3].type = 2;
routine_list[3].target = "愤怒,370008,80";
routine_list[3].reward = "0|0|0|每日任务礼包,588451,1";
...
function routine_create()
	if(routine_index == nil)then
		...
		Char.SetData(routine_index,%对象_形象%,14724);
		Char.SetData(routine_index,%对象_原形%,14724);
		Char.SetData(routine_index,%对象_X%,490);
		Char.SetData(routine_index,%对象_Y%,201);
		Char.SetData(routine_index,%对象_地图%,100);
...
--			routine_offer( _index_me, _index_tome);
		if( argument == 1)then
			if(Char.GetData(_index_tome,%对象_等级%) <=9)then
				NLG.ShowWindowTalked( _index_tome, _index_me, %窗口_信息框%, %按钮_关闭%, 
				ROUTINE_WINDOW_SEQUENCE_CANCEL, "您的等级小于10级，无法接任务。");
				return;
			end
			routine_offer( _index_me, _index_tome)
